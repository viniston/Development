 ...
 *
 * all te backbone events and models presence here
 *
...
$(document).ready(function () {
    ...
    var AllStation = [], formMOdel = {}, formData = {}, Notifystatementsdisplaytime = '3';
 ...
 **/
    ...
    window.AppView = Backbone.View.extend({
        ...
        stateSelected: function () {
            formData["sel_State"] = $('#select2-state').val(); $("#select2-city").select2("val", "-Select City-"); $("#stationcode").val(''); this.populateCity();
        },
        ...
        populateCity: function () {
            var cities = []; cities = $.grep(AllStation, function (rel) { return rel.StateName == formData["sel_State"]; });
            $('#select2-city option').remove(); $('#select2-city').append($("<option></option>").attr("value", 'Select City-').text('Select City-'));
            if (cities.length > 0) for (var i = 0, city = {}; city = cities[i++];) { $('#select2-city').append($("<option></option>").attr("value", city.CityName).text(city.CityName)); }
        },

        // Auto calculation of the variance (variance = actual-predicted speed) ...

        calculateVariance: function () {
            var predictedSeed = $("#predictedspeed").val(), actualSpeed = $("#actualspeed").val(), variance = (actualSpeed != '' ? actualSpeed : 0) - (predictedSeed != '' ? predictedSeed : 0); $("#variancenumber").val(variance);
        ...
        populateStationCode: function () {
            formData["sel_State"] = $('#select2-state').val(); formData["sel_city"] = $('#select2-city').val();
            var station = $.grep(AllStation, function (rel) { return rel.StateName == formData["sel_State"] && rel.CityName == formData["sel_city"]; })[0];
            if (station != null) { $("#stationcode").val(station.StationID); $("#predictedspeed").val(station.PredictedSpeed); } else { $("#stationcode").val(''); $("#predictedspeed").val(''); }
        },
        ...
        generateCaptchaImage: function () {
            ...
            $.ajax({
                url: "api/Authorization/GeneratingCaptchaCookie", type: "POST",
                data: { "Location": "Wnreg" }, dataType: "json",
                success: function (e) { $("#UserRegCaptcha").val(""); $("#imgCaptcha").attr("src", "GenerateCaptcha.aspx?purpose=Wnreg&time=" + rString); },
                error: function (e, o, t) { bootbox.alert(e + "\n" + o + "\n" + t) }
            });
            return false;
        },
        ...
        initInputForm: function () {
            $('select').select2(); $('#entrydate').datepicker({ format: "MM dd, yyyy", language: 'en' });
            $('[title!=""]').qtip({ style: { classes: 'qtip-dark qtip-shadow qtip-rounded"' } }); this.GetValidationList(); this.generateCaptchaImage();
            $.ajax({
                url: "api/Common/GetStations", type: "GET", dataType: "json", async: false,
                success: function (e) {
                    AllStation = e.Response; var States = []; $.each(e.Response, function (index, value) {
                        if ($.inArray(value.StateName, States) === -1) {
                            States.push(value.StateName);
                        }
                    }); for (var i = 0, state = {}; state = States[i++];) { $('#select2-state').append($("<option></option>").attr("value", state).text(state)); }
        ...
        historicalData: function () {
            this.sowhistory();
            $.ajax({
                url: "api/Common/GetHistoricalData/10", type: "GET",
                success: function (e) {
        ...
        expandcollpasehistoryblock: function (e) {
            ...
            if (isExpand) {
                $(".historical-wrapper").hide(); $("#expandcollapse").removeClass("icon-minus infoIcon").addClass("icon-plus infoIcon");
            }
            else {
                $(".historical-wrapper").show(); $("#expandcollapse").removeClass("icon-plus infoIcon").addClass("icon-minus infoIcon");
            }
        },
